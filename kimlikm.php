...
if($_POST){
    ...
    if($OkulBilgi){
        ...
        $Ogrenciler=$db->query("SELECT * FROM ogrenci WHERE sinif='{$_POST['sinif']}' AND okulid='{$id}'",PDO::FETCH_ASSOC);
        $Siniflar = $db->query("SELECT DISTINCT sinif AS Sinif FROM ogrenci WHERE okulid='{$id}'", PDO::FETCH_ASSOC);
    ...
    <div class="panel">
        <form action="kimlikm.php" method="post">
            <input type="hidden" value="<?php echo $id; ?>" name="okulid" />
