...
if($_POST){
    ...
    $id=intval($_POST['okulid']);
    $kimlik=intval($_POST['kimlik']);
    $eksikler=explode(',',$_POST['eksikler']);


    foreach($eksikler as $eksno) {
        $Eksikler = $db->query("SELECT * FROM ogrenci WHERE okulid='{$id}' AND numara='{$eksno}'")->fetch(PDO::FETCH_ASSOC);
        if(!empty($Eksikler)) {
            $Biriktir[] = $Eksikler;
        }
    }

    $KimlikBilgi=$db->query("SELECT * FROM kimlik WHERE id='{$kimlik}' AND okulid='{$id}'")->fetch(PDO::FETCH_ASSOC);
...
?>
    ...
    <head>
        <!-- BASICS -->
        <title><?php echo $OkulBilgi['okuladi'].' Eksikleri - '.date('d.m.Y'); ?></title>
